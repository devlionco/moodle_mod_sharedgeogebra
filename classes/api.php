...
/**
 * The mod_sharedgeogebra sharedgeogebra API.
 *
...
class api {
     ...
     * @return string
     */
    public function getsessionurl(string $urlparam, string $sharedgeogebra) {
        return $this->baseurl . "users/set-session/?token={$this->getcustomertoken()}&url_param={$urlparam}&map={$sharedgeogebra}";
    }
    ...
    private function request(string $url, array $body): array {
        ...
        if ($curl->error) {
            throw new \moodle_exception('curlerror', 'mod_sharedgeogebra', '', $curl->error);
        }

        $decoded = json_decode($response, true);

        if ($decoded['status'] == false) {
            throw new \moodle_exception('curlerror', 'mod_sharedgeogebra', '', $decoded['message']);
        }
